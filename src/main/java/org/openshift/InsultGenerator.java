...
public class InsultGenerator {
	public String generateInsult() {
		...
		String article = "an";
		String theInsult = "";
		// String firstAdjective = words[0][new
		// Random().nextInt(words[0].length)];
		// String secondAdjective = words[1][new
		...
		try {
			...
			if (connection != null) {
				String SQL = "select a.string AS first, b.string AS second, c.string AS nount from ";
				SQL += "short_adjective a, long_adjective b, noun c ORDER BY random() limit 1";
				...
				while (rs.next()) {
					...
					theInsult = String.format("Thou art %s %s %s!", article, rs.getString("first"),
